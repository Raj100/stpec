...
const NewsTable = () => {
  ...
  const { notices,newsUploadStatus } = useSelector((state) => state.notices);
  let news = notices;
  ...
  const handleSubmitNews = async (e) => {
    e.preventDefault();
    if (editNewsMode) {
      dispatch(editNotice({ id: currentNewsId, updatedNews: newNews }));
      setEditNewsMode(false);
    } else {
      await dispatch(postNotice(newNews));
    }
    resetNewsForm();
    fetchData();
  };
  ...
  return (
    <>
      <h2 className="text-lg font-bold mb-4 mt-8">NoticeBoard</h2>
      {
        (
